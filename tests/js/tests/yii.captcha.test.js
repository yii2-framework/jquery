...
describe("yii.captcha", function () {
    ...
    withData(
      {
        ...
        "no method specified, custom options": [
          function () {
      ...
      function (initFunction, expectedSettings) {
        it("should save settings for all elements", function () {
          initFunction();
          assert.deepEqual($("#captcha").data("yiiCaptcha"), {
            settings: expectedSettings,
          });
          assert.deepEqual($("#captcha-2").data("yiiCaptcha"), {
            settings: expectedSettings,
          });
        });
      },
  ...
  describe("refresh", function () {
    ...
    var response = {
      hash1: 747,
      hash2: 748,
      url: "/site/captcha?v=584696959e038",
    };
    ...
    withData(
      ...
      function (refreshFunction) {
        it("should send ajax request, update the image and data for client-side validation", function () {
          ...
          assert.include(
            server.requests[0].requestHeaders.Accept,
            "application/json",
          );
          assert.equal($captcha.attr("src"), response.url);
          assert.deepEqual($("body").data(settings.hashKey), [
            response.hash1,
            response.hash2,
          ]);
        });
