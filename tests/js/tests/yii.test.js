...
describe("yii", function () {
  ...
  before(function () {
    ...
    sinon = require("sinon");
    addPjaxAttributes();
    yiiGetBaseCurrentUrlStub = sinon.stub(
      yii,
      "getBaseCurrentUrl",
      function () {
        return "http://foo.bar";
  ...
  describe("confirm method", function () {
    ...
    function getConfirmCallbacksMessage(expectOkCalled, expectCancelCalled) {
      ...
      return messageMap[
        (expectOkCalled ? 1 : 0) + ":" + (expectCancelCalled ? 1 : 0)
      ];
    }
    ...
    withData(
      ...
      function (data) {
        ...
        var message =
          "should return undefined, confirm should be called once with according message, " +
        ...
        it(message, function () {
          ...
          assert.deepEqual(windowConfirmStub.getCall(0).args, [
            "Are you sure?",
          ]);
  ...
  describe("handleAction method", function () {
    ...
    describe("with no data-method", function () {
      var noActionsMessage =
        "should not do any actions related with page load and form submit";
      var pageLoadMessage =
        'should load new page using the link from "href" attribute';
      ...
      describe("with invalid elements or configuration", function () {
          ...
          withData(
            {
              ...
              'link, href contains anchor ("#") only, data-pjax': [
                ".link-anchor-href-pjax",
              ],
              // Not links
              ...
              "submit, data-form, form does not exist, data-pjax": [
                ".submit-form-not-exist-pjax",
              ],
            },
            function (elementSelector) {
              it(noActionsMessage, function () {
                var $element = $(
                  ".handle-action .no-method .invalid .no-form",
        ...
        describe("with form", function () {
          withData(
            {
              "not submit, data-form": [
                ".not-submit-outside-form",
                "#not-submit-separate-form",
              ],
              "not submit, inside a form": [
                ".not-submit-inside-form",
                "#not-submit-parent-form",
              ],
              "not submit, data-form, data-pjax": [
                ".not-submit-outside-form-pjax",
                "#not-submit-separate-form",
              ],
              "not submit, inside a form, data-pjax": [
                ".not-submit-inside-form-pjax",
                "#not-submit-parent-form-pjax",
              ],
            },
            function (elementSelector, formSelector) {
              it(noActionsMessage, function () {
                var $element = $(
                  ".handle-action .no-method .invalid .form",
                ).find(elementSelector);
                assert.lengthOf($element, 1);
      ...
      describe("with valid elements and configuration", function () {
          ...
          withData(
            ...
            function (elementSelector) {
              it(pageLoadMessage, function () {
                var $element = $(".handle-action .no-method .valid").find(
                  elementSelector,
                );
                assert.lengthOf($element, 1);
          ...
          withData(
            {
              ...
              "link, data-pjax, outside a container": [
                ".link-pjax-outside-container",
                "#pjax-separate-container",
              ],
              "link href, data-pjax, inside a container": [
                ".link-pjax-inside-container",
                "#pjax-container-2",
              ],
            },
        ...
        describe("with form", function () {
          it(
            pageLoadMessage +
              " when link is inside a form with explicit method",
            function () {
              ...
              assert.isAtLeast(
                windowLocationAssignStub.callCount,
                initialAssignCalls + 1,
              );
              assert.isTrue(
                windowLocationAssignStub.calledWith("/tests/index"),
              );
              assert.isFalse(pjaxClickStub.called);
              assert.equal(formSubmitsCount, 0);
              assert.isFalse(pjaxSubmitStub.called);
          ...
          withData(
            ...
            function (elementSelector, formSelector) {
              it("should submit according existing form", function () {
                ...
                assert.equal(
                  $savedSubmittedForm.get(0).outerHTML,
                  initialFormHtml,
                );
              });
            },
          ...
          withData(
            {
              "submit, data-form, data-pjax": [
                ".submit-outside-form-pjax",
                "#submit-separate-form",
              ],
              "submit, inside a form, data-pjax": [
                ".submit-inside-form-pjax",
                "#submit-parent-form-pjax",
              ],
            },
            function (elementSelector, formSelector) {
      ...
      describe("with no form", function () {
        withData(
          ...
          function (elementSelector, expectedFormHtml) {
            it("should create temporary form and submit it", function () {
              var $element = $(".handle-action .method .no-form").find(
                elementSelector,
              );
              assert.lengthOf($element, 1);
        ...
        describe('with data-method="get", data-params, data-pjax', function () {
          it("should create temporary form and submit it with pjax", function () {
            var event = $.Event("click");
            var $element = $(
              ".handle-action .method .no-form .get-params-pjax",
            );
            assert.lengthOf($element, 1);
            ...
            var expectedFormHtml =
              '<form method="get" action="/tests/index" style="display: none;">' +
              '<input name="foo" value="1" type="hidden" data-yii-action-helper="true">' +
              '<input name="bar" value="2" type="hidden" data-yii-action-helper="true">' +
              "</form>";
            assert.equal(
      ...
      describe("with form", function () {
        withData(
          ...
          function (elementSelector, formSelector, expectedSubmittedFormHtml) {
            var message =
              "should modify according existing form, submit it and restore to initial condition";
            it(message, function () {
              var $element = $(".handle-action .method .form").find(
                elementSelector,
              );
              assert.lengthOf($element, 1);
              ...
              var submittedFormHtml = StringUtils.cleanHTML(
                $savedSubmittedForm.get(0).outerHTML,
              );
              ...
              assert.strictEqual(
                $form.data("yiiActiveForm").submitObject,
                $element,
              );
            });
          },
        ...
        describe("with data-form, new action, new method, data-params, data-pjax", function () {
          var message =
            "should modify according existing form, submit it with pjax and restore to " +
            " initial condition";
          it(message, function () {
            var event = $.Event("click");
            var $element = $(
              ".handle-action .method .form .new-action-new-method-pjax",
            );
            assert.lengthOf($element, 1);
    ...
    withData(
      {
        ...
        'query parameter value with "=" signs': [
          "/posts/index?token=a=b=&bar=2",
          { token: "a=b=", bar: "2" },
        ],
        ...
        "skipped values": [
          "?foo=&PostSearch[tags][]=1&PostSearch[tags][]=",
          { foo: "", "PostSearch[tags][]": ["1", ""] },
        ],
        "encoded URI component": [
          "/posts/index?query=" + encodeURIComponent("count >= 1"),
          { query: "count >= 1" },
        ],
        ...
        'encoded URI component, "+" signs': [
          "/posts/index?next+celebration+day=Sunday+January+1st&" +
            ...
            encodeURIComponent("%"),
          {
            "next celebration day": "Sunday January 1st",
            "increase ++ %": "++ 20 %",
          },
        ],
  ...
  describe("initModule method", function () {
    ...
    withData(
      ...
      function (
        rootModuleIsActive,
        rootModuleInitParam,
        expectedCalledInitMethods,
      ) {
        var message =
          "should call init method in the root module and all submodules depending depending on " +
  ...
  describe("CSRF handler", function () {
    ...
    withData(
      {
        "crossDomain is false, csrfParam is not set": [
          false,
          undefined,
          undefined,
        ],
        "crossDomain is false, csrfParam is set": [false, "foobar", "foobar"],
        "crossDomain is true, csrfParam is not set": [
          true,
          undefined,
          undefined,
        ],
      ...
      function (crossDomain, csrfParam, expectedHeaderValue) {
        var message =
          "should add header with CSRF token to AJAX requests only when crossDomain is false and " +
      ...
      describe("with custom header set", function () {
        it("should perform redirect", function () {
          ...
          server.requests[0].respond(
            200,
            { "X-Redirect": "http://redirect.yii" },
            "",
          );

          assert.lengthOf(server.requests, 1);
          assert.isTrue(windowLocationAssignStub.calledOnce);
          assert.deepEqual(windowLocationAssignStub.getCall(0).args, [
            "http://redirect.yii",
          ]);
        });
      });
          ...
          withData(
            {
              ...
              "absolute url, wildcard": [
                "http://foo.bar/js/reloadable/script2.js?v=2",
              ],
            },
            function (url) {
              it("should load it as many times as it was requested", function () {
          ...
          withData(
            {
              ...
              "relative url, all wildcards are empty": [
                "/js/reloadable/script.js?v=",
              ],
              "absolute url": [
                "http://foo.bar/js/reloadable/not_reloadable_script.js",
              ],
              "absolute url, 1 empty wildcard": [
                "http://foo.bar/js/reloadable/script1.js?v=",
              ],
            },
            function (url) {
              it("should load it only once for both relative and absolute urls", function () {
  ...
  describe("data methods", function () {
    ...
    describe("with clickableSelector with data-confirm", function () {
      it("should call confirm and handleAction methods", function () {
        ...
        assert.instanceOf(
          yiiConfirmSpy.getCall(0).thisValue,
