...
describe("yii.gridView", function () {
  ...
  var gridViewHtml = fs.readFileSync(
    ...
    withData(
      {
        ...
        "no method specified, custom settings": [
          function () {
      ...
      function (initFunction, expectedSettings) {
        it("should save settings for all elements", function () {
          initFunction();
          assert.deepEqual($("#w0").yiiGridView("data"), {
            settings: expectedSettings,
          });
          assert.deepEqual($("#w1").yiiGridView("data"), {
            settings: expectedSettings,
          });
        });
      },
  ...
  describe("applyFilter", function () {
    ...
    describe("with beforeFilter returning not false", function () {
      ...
      var message =
        "should send the request to correct url with correct parameters and apply events in " +
        "correct order";
      it(message, function () {
        $gridView = $("#w0")
        ...
        var expectedHtml =
          '<form action="/posts/index" method="get" class="gridview-filter-form" ' +
        ...
        assert.equal(
          $(beforeFilterSpy.getCall(0).args[0].target).attr("id"),
          $gridView.attr("id"),
        );

        assert.isTrue(jQuerySubmitStub.calledOnce);
        assert.equal(
          jQuerySubmitStub.returnValues[0].attr("class"),
        ...
        assert.equal(
          $(afterFilterSpy.getCall(0).args[0].target).attr("id"),
          $gridView.attr("id"),
        );
    ...
    describe("with different urls", function () {
      ...
      describe("with filter data sent", function () {
        it("should send the request to correct url with new parameter values", function () {
          var filterUrl =
            "/posts/index?CategorySearch[id]=5&CategorySearch[name]=c" +
          ...
          var expectedQueryString =
            "PostSearch[name]=b&PostSearch[category_id]=1" +
    ...
    describe("with list box", function () {
      describe("with values selected", function () {
        it("should send the request to correct url with correct parameters", function () {
          ...
          var $form = $gridView.find(".gridview-filter-form");
          var expectedQueryString =
            "PostSearch[name]=&PostSearch[tags]=-1&PostSearch[tags][]=1" +
            "&PostSearch[tags][]=2";
      ...
      describe("with unselected values after applied filter", function () {
        it("should send the request to correct url with correct parameters", function () {
          ...
          var filterUrl =
            "/posts/index/?PostSearch[name]=&PostSearch[tags]=-1&PostSearch[tags][]=1" +
            "&PostSearch[tags][]=2";
          $gridView = $("#w2").yiiGridView({
            filterUrl: filterUrl,
            filterSelector: "#w2-filters input, #w2-filters select",
          ...
          assert.equal(
            decodeURIComponent($form.serialize()),
            "PostSearch[name]=&PostSearch[tags]=-1",
          );
        });
      });
      ...
      withData(
        {
          ...
          "multiple and checkAll options, multiple set to false": [
            { multiple: false, checkAll: "selection_all" },
          ],
        },
        function (customOptions) {
          it('should update data and do not activate "check all" functionality', function () {
            ...
            assert.equal(
              $gridView.yiiGridView("data").selectionColumn,
              "selection[]",
            );
    ...
    withData(
      {
        ...
        "selectionColumn not set, 1st and 2nd rows selected": [
          undefined,
          [1, 2],
          false,
          [],
        ...
        "selectionColumn set, 1st row selected": [
          "selection[]",
          [1],
          false,
          [1],
        ],
        "selectionColumn set, 1st and 2nd rows selected": [
          "selection[]",
          [1, 2],
          false,
          [1, 2],
      ...
      function (selectionColumn, selectedRows, checkAll, expectedSelectedRows) {
        it("should return array with ids of selected rows", function () {
          ...
          assert.deepEqual(
            $gridView.yiiGridView("getSelectedRows"),
  ...
  describe("destroy method", function () {
    ...
    it("should remove saved settings for destroyed element only and return initial jQuery object", function () {
      ...
      assert.deepEqual($gridView2.yiiGridView("data"), {
    ...
    it('should remove "checkRow" and "checkAllRows" filter event handlers for destroyed element only', function () {
      ...
      interactions.click(
        $gridView2.find('input[name="selection[]"][value="1"]'),
      );
