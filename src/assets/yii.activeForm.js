...
(function ($) {
  ...
  var validateClientAttribute = function (
    $form,
    data,
    attribute,
    messages,
    ...
    if (
      shouldSkipClientValidation($input) ||
      !shouldValidateAttribute(data, attribute)
    ) {
      return false;
    }
  ...
  var completeValidation = function (
    $form,
    data,
    messages,
    deferreds,
  ...
  var finalizeValidationWithoutAjax = function (
    $form,
    data,
    messages,
    submitting,
  ...
  var validateAttribute = function (
    ...
    if (hasValueChanges) {
      $.each(data.attributes, function () {
        if (
          (!this.hasWhenClient && !this.enableAjaxValidation) ||
          this.status === 2 ||
          this.status === 3
        ) {
          return;
  ...
  var updateInputs = function ($form, messages, submitting) {
    ...
    var errorAttributes = collectErrorAttributes(
      $form,
      data,
      data.attributes,
      messages,
  ...
  var collectErrorAttributes = function (
    $form,
    data,
    attributes,
    messages,
  ...
  var collectErrorAttribute = function (
    $form,
    data,
    attribute,
    messages,
  ...
  var hasAttributeError = function (
    $form,
    data,
    attribute,
    messages,
    ...
    if (
      attribute.status === 2 ||
      attribute.status === 3 ||
      messages[attribute.id] !== undefined
    ) {
  ...
  var handleSubmittingValidationResult = function (
    $form,
    data,
    messages,
    errorAttributes,
  ...
  var getFirstErrorTop = function ($form, errorAttributes, scrollOffset) {
    ...
    return (
      $form.find(firstErrorSelector).first().closest(":visible").offset().top -
      scrollOffset
    );
  };
