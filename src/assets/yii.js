...
window.yii = (function ($) {
  ...
  function submitActionForm($form, usePjax, pjaxOptions) {
    if (usePjax) {
      $form
        .off("submit.yiiPjaxSubmit")
        .on("submit.yiiPjaxSubmit", function (e) {
  ...
  function getQueryParamName(pair, pairSeparatorPosition) {
    return pairSeparatorPosition < 0
      ? pair
      : pair.substring(0, pairSeparatorPosition);
  }
  ...
  function shouldSkipDataMethod(actionData) {
    return (
      actionData.method === undefined &&
      actionData.message === undefined &&
      actionData.form === undefined
    );
  }
